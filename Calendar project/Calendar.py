        ...
        if start_time <= end_time:
            if date not in calendar:
                ...
                if len(calendar[date]) > 1:
                    calendar[date].sort(key=lambda x: x['start'])
            return True
        else:
            return False
    else:
...
def command_show(calendar):
    ...
    for date in sorted(calendar.keys(), reverse=True):
        calendar_show += '\n{} : '.format(date)
        calendar_sorted = sorted(calendar[date], key=lambda x: x['start'])
        for entry in calendar_sorted:
            calendar_show += '\n    start : {},\n    end : {},\n    title : {}'.format(datetime.time(entry['start']).
                                                                                       strftime("%H:%M"), datetime.time(
                entry['end']).strftime("%H:%M"), entry['title'])
            if len(calendar_sorted) > 1 and calendar_sorted.index(entry) < len(calendar[date]) - 1:
                calendar_show += '\n'
...
def command_delete(date, start_time, calendar):
    ...
    if date in calendar:
        if len(calendar[date]) != 0:
            for entry in calendar[date]:
                start_time_list = list()
                for x in calendar[date]:
                    start_time_list.append(x['start'])
                if start_time in start_time_list:
                    if len(calendar[date]) == 1:
                        del calendar[date]
                    else:
                        calendar[date].remove(entry)
                    return True
                else:
...
def save_calendar(calendar):
    ...
    # YOUR CODE GOES HERE
    import datetime
    with open('calendar.txt', 'w') as f1:
        for date in sorted(calendar, reverse=True):
            f1.write(date + ':')
            for entry in sorted(calendar[date], key=lambda x: x['start']):
                start = datetime.time(entry['start']).strftime('%H')
                end = datetime.time(entry['end']).strftime('%H')
                title = str(entry['title'])
                if len(calendar[date]) > 1 and calendar[date].index(entry) < len(calendar[date]) - 1:
                    f1.write(start + '-' + end + ' ' + title + '\t')
                elif len(calendar[date]) > 1 and calendar[date].index(entry) == len(calendar[date]) - 1:
                    f1.write(start + '-' + end + ' ' + title)
                elif len(calendar[date]) == 1:
                    f1.write(start + '-' + end + ' ' + title)
            f1.write('\n')
    return True
    pass
...
def load_calendar():
    ...
    if os.path.exists('calendar.txt'):
        with open('calendar.txt', 'r') as f1:
            cal = f1.readlines()
            for line in cal:
                temp = line.split(':')
                temp_1 = temp[1].split('\t')
                ...
                for i in temp_1:
                    ...
                    start_time = int(temp_2[0])
                    end_time = int(temp_3[0])
                    title = " ".join(temp_3[1:]).rstrip('\n')
                    command_add(date, start_time, end_time, title, calendar)
            return calendar
    else:
        open('calendar.txt', 'w').close()
        return calendar
    pass
...
def is_command(command):
    ...
    # YOUR CODE GOES HERE
    valid_command = ["add", "delete", "quit", "help", "show"]
...
def is_calendar_date(date):
    ...
    if len(date) != 10:
        return False
    year = date.split('-')[0]
    month_0 = date.split('-')[1]
    day_0 = date.split('-')[2]
    if len(year) == 4:
        if is_natural_number(month_0) and is_natural_number(day_0):
            month = int(month_0)
            day = int(day_0)
...
def parse_command(line):
    ...
    # the final argument.
    # YOUR CODE GOES HERE
    u_command = line.split(" ")
    if is_command(u_command[0]):
        if u_command[0] == 'add':
            if len(u_command) < 5:
                return ['error', 'add DATE START_TIME END_TIME DETAILS']
            if is_calendar_date(u_command[1]):
                title = " ".join(u_command[4:])
                return ['add', u_command[1], int(u_command[2]), int(u_command[3]), title]
            else:
        ...
        if u_command[0] == 'delete':
            if len(u_command) != 3:
                return ['error', 'delete DATE START_TIME']
            if is_calendar_date(u_command[1]):
                if is_natural_number(u_command[2]):
                    return ['delete', u_command[1], int(u_command[2])]
                else:
